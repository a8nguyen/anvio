...
class KMerTablesForContigsAndSplits:
    ...
    def get_kmer_freq(self, sequence):
        return self.kmers_class.get_kmer_frequency2(sequence, dist_metric_safe=True)


    def append(self, seq_id, sequence, kmer_freq=None):
        if not kmer_freq:
            kmer_freq = self.kmers_class.get_kmer_frequency2(sequence, dist_metric_safe=True)
